...
describe(createKitchen, () => {
    describe("announce", () => {
        it("announces the starting state", () => {
            ...
            expect(result).toBe(`I have 0 much dirt, 123 budget, 0 bread(s), 0 fruit(s), 0 sauce(s), and 0 vegetable(s).`);
        });
    });
    describe("clean", () => {
    ...
    describe("purchase", () => {
        it("returns false without changing stock when expense is greater than budget", () => {
            ...
            expect(kitchen.announce()).toBe("I have 0 much dirt, 1 budget, 0 bread(s), 0 fruit(s), 0 sauce(s), and 0 vegetable(s).");
        });
        it("returns true and changes stock when expense is greater than budget", () => {
            const supplier = jest.fn((expense) => ({
                breads: expense * 1,
                fruits: expense * 2,
                sauces: expense * 3,
            ...
            expect(kitchen.announce()).toBe("I have 0 much dirt, 2 budget, 3 bread(s), 6 fruit(s), 9 sauce(s), and 12 vegetable(s).");
        });
        ...
        it("returns false without changing stock when expense is less than budget on a second call", () => {
            ...
            const result = kitchen.purchase(2);
            expect(result).toBe(true);
            expect(kitchen.announce()).toBe("I have 0 much dirt, 0 budget, 5 bread(s), 10 fruit(s), 15 sauce(s), and 20 vegetable(s).");
        });
    });
    describe("prepare", () => {
        it("returns true and decreases stock when dirt is below 100 and the recipe succeeds", () => {
            ...
            expect(result).toBe(true);
            expect(kitchen.announce()).toBe("I have 1 much dirt, 0 budget, 1 bread(s), 2 fruit(s), 3 sauce(s), and 4 vegetable(s).");
        });
        it("returns false and does not change stock when dirt is below 100 and the recipe fails", () => {
            ...
            expect(result).toBe(false);
            expect(kitchen.announce()).toBe("I have 1 much dirt, 0 budget, 0 bread(s), 0 fruit(s), 0 sauce(s), and 0 vegetable(s).");
        });
        it("returns false and does not call recipe when dirt is 100", () => {
            ...
            expect(result).toBe(false);
            expect(kitchen.announce()).toBe("I have 100 much dirt, 0 budget, 1 bread(s), 2 fruit(s), 3 sauce(s), and 4 vegetable(s).");
        });
    });
});
