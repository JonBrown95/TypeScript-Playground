...
for (const chapterSlug of fs.readdirSync("projects")) {
    describe(chapterSlug, () => {
        ...
        test("_category_.json", () => {
            const categoryData = readFileAsJSON(`${chapterDirectory}/_category_.json`);
        ...
        for (const projectSlug of projectSlugs) {
            describe(projectSlug, () => {
                const projectContents = fs.readdirSync(`projects/${chapterSlug}/${projectSlug}`);
                ...
                function testAppetizerProject() {
                    ...
                    test("README.md", () => {
                        ...
                        expect(contents).toContain(`> A [Learning TypeScript > ${chapterTitle}](https://learning-typescript.com/${chapterSlug}) 🥗 appetizer project.`);
...
`);
                            if (contents.includes("run Jest")) {
                                expect(contents).toContain("In one terminal, run the TypeScript compiler");
                                expect(contents).toContain(`In another terminal, run Jest via the \`test\` script on whichever step you're working on.
...
\`\`\``);
                    ...
                    if (chapterSlug !== "type-operations") {
                        test("tsconfig.json", () => {
                            const tsconfigData = readFileAsJSON(`${chapterDirectory}/${projectSlug}//tsconfig.json`);
                    ...
                    for (const stepSlug of stepSlugs) {
                        describe(stepSlug, () => {
                            test("tsconfig.json", () => {
                                const tsconfigData = readFileAsJSON(`${chapterDirectory}/${projectSlug}/${stepSlug}/tsconfig.json`);
                                expect(tsconfigData).toMatchObject({
                                    ...(chapterSlug === "configuration-options"
                                        ? {}
                                        : { extends: "../../../../tsconfig.json" }),
                ...
                function testEntreeOrDessertProject() {
                    const categoryData = readFileAsJSON(`${chapterDirectory}/${projectSlug}/_category_.json`);
                    ...
                    test("README.md", () => {
                        const contents = fs
                            ...
                            .toString();
                        expect(contents).toContain(`# ${projectTitle}`);
                        expect(contents).toContain(`> A [Learning TypeScript > ${chapterTitle}](https://learning-typescript.com/${chapterSlug}) ${mealEmoji} ${mealType} project.`);
                        expect(contents).toContain(`## Setup`);
                        expect(contents)
...
`);
                        if (contents.includes("run Jest")) {
                            expect(contents).toContain(`In another terminal, run Jest via the \`test\` script.
...
\`\`\``);
                ...
                function testCategoryJson(emoji) {
                    const categoryData = readFileAsJSON(`${chapterDirectory}/${projectSlug}/_category_.json`);
                    expect(categoryData).toEqual({
                        label: expect.stringMatching(new RegExp(`${emoji} ${projectTitle}`, "i")),
                        position: expect.any(Number),
                    });
                }
                function testPackageJson() {
                    test("package.json", () => {
                        const categoryData = readFileAsJSON(`${chapterDirectory}/${projectSlug}/package.json`);
                        expect(categoryData).toEqual({
                            name: projectSlug,
                            scripts: {
                        ...
                        const { test: testScript, "test:solutions": testSolutionsScript } = categoryData.scripts;
                        const solutionScripts = {
                            tsc: expect.stringMatching(/^tsc /),
                            jest: "cross-env TEST_SOLUTIONS=1 jest",
                        };
                        expect(testSolutionsScript).toEqual(solutionScripts[testScript] ?? "(unknown test script)");
                    });
                }
            });
        }
...
function toTitle(kebabCase) {
    return (capitalCase(kebabCase.replaceAll("-", " "))
